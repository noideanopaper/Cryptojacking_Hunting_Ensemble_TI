...
class Feature_engineering(object):
    ...
    def __init__(self):
        ...
        self.pat_list = {"btc": self.wallet_pattern_btc, "ltc": self.wallet_pattern_ltc, "xmr": self.wallet_pattern_xmr,
                         "paths": self.path_pattern,
                         "regs": self.regs_pattern, "urls": self.urls_pattern, "ips": self.ip_pattern,
                         "mz": self.mz_pattern,  # "other": self.strings_pattern,
                         "pe": self.pe_pattern, "pool": self.pool_pattern, "cpu": self.cpu_pattern,
                         "gpu": self.gpu_pattern, 'coin': self.coin_pattern}

        self.pat_list_filesize = {
            "regs": self.regs_pattern, "ips": self.ip_pattern,
            "mz": self.mz_pattern,  # "other": self.strings_pattern,
            "pe": self.pe_pattern, "pool": self.pool_pattern, "cpu": self.cpu_pattern,
            "gpu": self.gpu_pattern, 'coin': self.coin_pattern}
    ...
    def get_section_infomation(self, filedata):
        ...
        # readable、writable、executable sections size mean
        sR, sW, sX = [], [], []
        # readable、writable、executable sections entropy mean
        entrR, entrW, entrX = [], [], []
        # num of .rsrc
        rsrc_num = 0
        for s in lief_binary.sections:
        ...
        section_info['entr_R'], section_info['entr_W'], section_info['entr_X'] = np.mean(entrR), np.mean(
            entrW), np.mean(entrX)
...
#                 #     print(tmp)
                ...
                # print('len(tmp): k::{}{}'.format(k, len(tmp)))
                sample_df["{}_mean".format(k)] = np.mean([len(i) for i in tmp])
                # print('mean(tmp):k::{} {}'.format(k, sample_df["{}_mean".format(k)]))
        return sample_df
    ...
    def opcodes(self, filedata):
        ...
        # print('opcode_reg:{}'.format(pats))
        if len(pats) != 0:
            pt_uniq = set()
            pt_len = [len(opc) for opc in pats]
            # print("pt_len:{}".format(pt_len))

            for opc in pats:
                pt_uniq |= set(opc)
            sample_df["opcode_min"], sample_df["opcode_max"] = min(pt_len), max(pt_len)
            # print("opcode_min:{}  opcode_max:{}".format(min(pt_len), max(pt_len)))
    ...
    def get_feature_engineering(self, sample_data):
        ...
        # print('tmp_match: ', tmp_match)
        
        fs = tmp_section['file_size']/1024
        tar_keys = ['pool_mean','cpu_mean', 'gpu_mean', 'coin_mean', 'ips_mean', 'mz_mean', 'pe_mean', 'regs_mean']

        for key_cell in tar_keys:
            tmp_match[key_cell] = tmp_match[key_cell]/fs
        # print(fs)
        # print('tmp_match: ', tmp_match)
        # 0.24
